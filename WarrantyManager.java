...
public class WarrantyManager {
    // [Q&A #11] "Why constructor-inject ApplianceManager but pass
    // CustomerManager/StaffManager as method params instead?" ApplianceManager
    // is a structural dependency — nearly every method here needs the
    // inventory, so it's held as a permanent field. CustomerManager/
    // StaffManager are only needed for the duration of a single call (e.g.
    // processSale below), so they're passed per-call instead of stored,
    // keeping the field list honest about what's actually always needed.
